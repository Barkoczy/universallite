...
final class PageRouteList extends RouteList
{
  ...
  {
    // @confPath
    $confPath = Folder::getConfigPath().'/pages.yml';

    // @validate
    if (!is_file($confPath))
      throw new CannotReadFileFromFileSource('Cannot read aplication pages yaml configuration file due to insufficient permissions');
